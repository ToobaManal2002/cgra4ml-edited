...
def export_inference(model, hw, batch_size=1):
    ...
    with open (f'./config_fw.h', 'w') as ch:
        ...
        for ib, b in enumerate(BUNDLES):
            ...
            bias_size = b.be.size if b.core.b else 0  #new addition
            ln_gamma_offset = 0
            ln_beta_offset = 0
            if hasattr(b, 'layernorm') and b.layernorm and b.ln_gamma is not None:
                ln_gamma_offset = b_words + bias_size
                ln_beta_offset = b_words + bias_size +  len(b.ln_gamma)
            ch.write(     f".ln_gamma_offset={ln_gamma_offset:<5}, .ln_beta_offset={ln_beta_offset:<5}, ")
            ...
            ch.write(     f".is_bias={1*(b.core.b is not None):<3}, .is_flatten={1*(b.flatten is not None):<3}, .is_softmax={1*(b.softmax is not None):<3}, .is_gelu={1*(b.core.act.type == 'gelu'):<3}, .is_layernorm={1*(b.layernorm):<3}, ")   #new addition
            ...
            ch.write(     f".x_pad={b.r.X_PAD:<3}, .b_val_shift={b.core.bias_val_shift:<3}, .b_bias_shift={b.core.bias_b_shift:<3}, .ca_nzero={ca_nzero:<3}, .ca_shift={ca_shift:<3}, .ca_pl_scale={ca_pl_scale:<3}, .aa_nzero={aa_nzero:<3}, .aa_shift={aa_shift:<3}, .aa_pl_scale={aa_pl_scale:<3}, .pa_nzero={pa_nzero:<3}, .pa_shift={pa_shift:<3}, .pa_pl_scale={pa_pl_scale:<3}, .softmax_frac={b.softmax_frac:<3}, .ca_frac={getattr(b.core.act, 'in_frac', 0):<3}, .ln_frac={getattr(b, 'ln_frac', 0):<3}, ")
            ...
            if hasattr(b, 'layernorm') and b.layernorm and b.ln_gamma is not None:
                b_words += len(b.ln_gamma) + len(b.ln_beta)
        ...
        for ib, b in enumerate(BUNDLES):
            ...
            if hasattr(b, 'layernorm') and b.layernorm and b.ln_gamma is not None:  #new addition
                # Must be the SAME frac the C code divides by in apply_layernorm(),
                # i.e. the .ln_frac we just wrote into config_fw.h.
                ln_frac = b.ln_frac
                
                gamma_int = np.round(b.ln_gamma * (2**ln_frac)).astype(type_d['np'][hw.B_BITS])
                beta_int = np.round(b.ln_beta * (2**ln_frac)).astype(type_d['np'][hw.B_BITS])
                b_bitstring += gamma_int.tobytes()
                b_bitstring += beta_int.tobytes()                                       #end
